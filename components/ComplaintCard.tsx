...
const ComplaintCard: React.FC<ComplaintCardProps> = ({ complaint, onUpdate }) => {
    ...
    const handleReopen = async () => {
        setIsSaving(true);
        try {
            await updateComplaint(complaint.id, { status: ComplaintStatus.Reopened, solutionText: '' });
            addNotification('Complaint reopened successfully.', 'success');
            onUpdate();
        } catch (error) {
            addNotification('Failed to reopen complaint.', 'error');
            console.error(error);
        } finally {
            setIsSaving(false);
        }
    ...
    const handleStudentClose = async () => {
        setIsSaving(true);
        try {
            await updateComplaint(complaint.id, { status: ComplaintStatus.Closed });
            addNotification('Complaint closed successfully.', 'success');
            onUpdate();
            setIsOpen(false);
        } catch (error) {
            addNotification('Failed to close complaint.', 'error');
            console.error(error);
        } finally {
            setIsSaving(false);
        }
